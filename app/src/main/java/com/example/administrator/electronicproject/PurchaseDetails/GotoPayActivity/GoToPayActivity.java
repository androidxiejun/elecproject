package com.example.administrator.electronicproject.PurchaseDetails.GotoPayActivity;
...
import com.example.administrator.electronicproject.PurchaseDetails.Alipay.PayResult;
import com.example.administrator.electronicproject.PurchaseDetails.Alipay.SignUtils;
...
public class GoToPayActivity extends AppCompatActivity implements View.OnClickListener{
    @BindView(R.id.alipay_radio_group)
    RadioGroup mRadioGroup;
    @BindView(R.id.alipay_pay)
    Button mPayBtn;
    @BindView(R.id.alipay_back_btn)
    Button mBackBtn;
    ...
    public void onClick(View view) {
        if(view.getId()==R.id.alipay_back_btn){
            finish();
        }else{
            /////-----------------------------------需要放在后台服务器 begin-----------------------------------
            String orderInfo = getOrderInfo(title, "明星衣橱，全球好品质", "0.01");
            ...
            //-----------------------------------------后台服务器 end--------------------------------------
            Log.i("androidxx", "click: ");
            new Thread(new Runnable() {
                @Override
                public void run() {
                    //1、生成订单数据
                    //2、支付
                    PayTask payTask = new PayTask(GoToPayActivity.this);
                /*
                参数1：订单信息
